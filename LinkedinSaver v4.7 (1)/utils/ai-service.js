...
async function initAIService() {
  try {
    ...
    let storedOptInStatus = result.aiServiceOptIn; // Hole den gespeicherten Wert
    ...
    if (storedOptInStatus === undefined || storedOptInStatus === false) {
      userOptInStatus = true; // Setze den aktiven Status auf true
      await chrome.storage.local.set({ 'aiServiceOptIn': true }); // Speichere true dauerhaft für Tests
      console.log('AI Service: Opt-in automatisch für Tests aktiviert und gespeichert.');
    } else {
      userOptInStatus = storedOptInStatus; // Ansonsten den gespeicherten Wert verwenden
      console.log(`AI Service: Opt-in Status aus Storage geladen: ${userOptInStatus}`);
    }
