  ...
  function renderPosts() {
    ...
    filteredPosts.forEach(post => {
      ...
      postCard.innerHTML = `
        ...
        <div class="post-content">${PostProcessor.generateSummary(post.content, 300)}</div>
  ...
  function displayPostInModal(post) {
    ...
    modalContentEl.innerHTML = `
        ...
        ${post.images && post.images.length > 0 ? `<div class="modal-post-images">
            ${post.images.map(imgSrc => `<img src="${imgSrc}" alt="Post image" style="max-width: 100%; max-height: 300px; object-fit: contain; margin-right: 5px; border-radius: 4px;">`).join('')}
        </div>` : ''}
