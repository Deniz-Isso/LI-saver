...
function extractDataFromPostElement(postElement) {
  ...
  try {
    ...
    // Priorisiere den Link aus dem "Permalink"-Element (falls vorhanden)
    // Es gibt verschiedene Selektoren für den Permalink, hier die gängigsten prüfen
    const permalinkSelectors = [
      'a[data-test-id="main-feed-activity-card__permalink"]', // Häufig auf Feed-Seiten
      '.feed-shared-control-menu__control-item[href*="/posts/"]', // Allgemeine Permalink-Links
      '.update-components-control-menu__control-item[href*="/posts/"]',
      '.feed-shared-control-menu__control-item[href*="/feed/update/"]',
      '.update-components-control-menu__control-item[href*="/feed/update/"]',
      'a[data-control-name="permalink"]', // Ein älterer, aber noch relevanter Selector
      '.feed-shared-header__link', // Neuere LinkedIn Struktur
      'a[href*="/posts/"]', // Generischer Posts-Link
      'a[href*="/feed/update/"]', // Feed Update Links
      '.feed-shared-actor__meta a', // Author meta links
      'time[datetime] ~ a', // Links neben Zeitstempel
      '[data-urn] a[href*="linkedin.com"]' // Links in Post-Containern mit URN
    ];

    for (const selector of permalinkSelectors) {
      const permalinkElement = postElement.querySelector(selector);
      if (permalinkElement && permalinkElement.href && permalinkElement.href.includes('linkedin.com')) {
        postUrl = permalinkElement.href;
        console.log('Extractor: Found permalink URL:', postUrl);
        break; // Sobald ein Link gefunden, Schleife beenden
      }
    }

    // Alternative: Versuche Post-URL aus Post-ID oder Author-Profil zu konstruieren
    if (!postUrl) {
      // Versuche Post-ID aus data-urn zu extrahieren
      ...
      if (urnElement) {
        ...
        if (activityMatch) {
          ...
          console.log('Extractor: Constructed URL from activity URN:', postUrl);
        }
      }
    }

    // Fallback: Nehme die aktuelle URL nur als letzte Option
    if (!postUrl && (window.location.href.includes('linkedin.com/feed') || window.location.href.includes('linkedin.com/posts'))) {
      postUrl = window.location.href;
      console.log('Extractor: Using current window URL as fallback:', postUrl);
    }

    // Stelle sicher, dass die URL über Storage.normalizeUrl bereinigt wird
    if (postUrl) {
        // Annahme: Storage.normalizeUrl ist bereits verfügbar oder wird importiert.
        // Wenn nicht, muss `import * as Storage from '../utils/storage.js';` hinzugefügt werden.
        if (typeof Storage !== 'undefined' && Storage.normalizeUrl) {
          postUrl = Storage.normalizeUrl(postUrl); 
        }
        console.log('Extractor: Normalized URL:', postUrl);
    }
