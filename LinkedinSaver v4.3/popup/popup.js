...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const postImagesEl = document.getElementById('post-images');
  ...
  const newFolderInputEl = document.getElementById('new-folder-input');
  ...
  if (addFolderBtn) {
    addFolderBtn.addEventListener('click', () => {
      ...
      document.getElementById('new-folder-section').classList.toggle('hidden');
      if (newFolderInputEl) newFolderInputEl.focus();
    });
  }
  
  // Save folder button for auto tab
  const saveFolderBtn = document.getElementById('save-folder-button');
  if (saveFolderBtn) {
    saveFolderBtn.addEventListener('click', async () => {
      console.log('Save folder clicked');
      const folderName = newFolderInputEl.value.trim();
      if (folderName) {
        await saveFolder(folderName);
        newFolderInputEl.value = '';
        document.getElementById('new-folder-section').classList.add('hidden');
        await updateFolderSelects();
      }
    });
  }
  ...
  if (manualAddFolderBtn) {
    manualAddFolderBtn.addEventListener('click', () => {
      ...
      document.getElementById('manual-new-folder-section').classList.toggle('hidden');
  ...
  if (manualSaveFolderBtn) {
    manualSaveFolderBtn.addEventListener('click', async () => {
      ...
      if (folderName) {
        ...
        document.getElementById('manual-new-folder-section').classList.add('hidden');
        await updateFolderSelects();
      }
    });
  }
  ...
  function handleExtractedPostData(postData) {
    ...
    currentPostData = {
      ...
      link: postData.link || window.location.href,
